...
int get_str(char* str) //читаем строчку со стандартного потока
{
    char c;
    int i = 0;
    while ( (c=getchar()) != '\n' )
...
int get_cmd(char *condcmd);


 
int get_condcmd(char* str)   //по исходной строчке в cmd делает команду с условным выполнением  (парсим до & или до ;)
{
    int k1 = 0;
...
int get_cmd(char *str) //по команде с условным выполнением делает команду (парсим до && или до ||)  
{
    int k1 = 0;
    if (str == NULL) return 1;
    size_t start = 0;
...
int get_simplecmd(char* str, int fd1, int fd2) //по конвейеру получаем простую команду, которую и будем загружать в exec
{
    //    printf("-------------------------- here2\n");
   ...
   // printf("vhod prostoi komande %s\n", str);
    ...
    for (int i = 1; i < cnt;)
    {
        ...
        if (string[end] == '|')
        {
            ...
            start = end;
            pipe(fd); // канал между текуще и следующей командой
...
int execute_cmd(char** matrix, int fd1, int fd2) //выполняем команду, учитывая перенаправление ввода-вывода
{
    //    printf("-------------------------- here3\n");
